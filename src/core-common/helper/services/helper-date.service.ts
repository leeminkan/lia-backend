...
export class HelperDateService implements IHelperDateService {
  ...
  constructor(private readonly configService: ConfigService<AllConfigType>) {
    this.defTz = this.configService.getOrThrow('app.timezone', { infer: true });
  }
  ...
  diff(
    ...
    if (options?.format === HELPER_DATE_DIFF.MILIS) {
      return diff.asMilliseconds();
    } else if (options?.format === HELPER_DATE_DIFF.SECONDS) {
      return diff.asSeconds();
    } else if (options?.format === HELPER_DATE_DIFF.HOURS) {
      return diff.asHours();
    } else if (options?.format === HELPER_DATE_DIFF.MINUTES) {
  ...
  format(date: Date, options?: IHelperDateFormatOptions): string {
    ...
    return moment(date)
      .tz(this.defTz)
      .format(options?.format ?? HELPER_DATE_FORMAT.DATE);
  }
